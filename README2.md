...
If advanced searches are hard to understand at first, simple filtering is a good alternative. Filtering will also teach you the [fields](https://www.ncbi.nlm.nih.gov/books/NBK49540/) which can be used to narrow your results. 

In the example below, I am using NCBI Nucleotide to search for the AIRE gene. 2,228 results are given, spanning all datatypes.  

<p align="center">
    <img src="media/nucleotide1.PNG" width="80%">
</p>

Here, we only want sequences which look like the AIRE gene. We can use filters to narrow our results - seen on the left side of the screen. By setting the 'Sequence length' filter to 1700 - 2000 bp, we can keep sequences which appear to be orthologues of the AIRE gene.  

If we wanted a specific set of organisms, we could select a specific taxonomic group (top right). 

The 'Search details' window shows what NCBI used to create the results list, and can be used for a more advanced search ([see this list of valid fields](https://www.ncbi.nlm.nih.gov/books/NBK49540/))
...
This narrows our results to 146 items. The results sequences can be downloaded by clicking 'Send to' in the top part of the results page. A number of options are available, and in this case I have selected FASTA to download the actual nucleotide sequences in FASTA format. 
